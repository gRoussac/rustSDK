...
impl BlockHash {
    ...
    pub fn new(block_hash_hex_str: &str) -> Result<BlockHash, JsError> {
        let bytes =
            decode(block_hash_hex_str).map_err(|err| JsError::new(&format!("{:?}", err)))?;
    ...
    pub fn from_digest(digest: Digest) -> Result<BlockHash, JsError> {
