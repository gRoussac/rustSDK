...
export class ActionComponent implements AfterViewInit, OnDestroy {
  ...
  async ngAfterViewInit(): Promise<void> {
    ...
    this.sdk_deploy_utils_methods = this.sdk_methods.filter((name) =>
      ['make_deploy', 'make_transfer', 'sign_deploy', 'put_deploy'].includes(
        name,
      ),
    );

    this.sdk_contract_methods = this.sdk_methods.filter((name) =>
      [
        'call_entrypoint',
        'install',
        'query_contract_dict',
    ...
    this.sdk_rpc_methods = this.sdk_methods.filter(
      (name) =>
        !this.sdk_deploy_methods
          .concat(this.sdk_deploy_utils_methods, this.sdk_contract_methods)
          .includes(name),
    );
