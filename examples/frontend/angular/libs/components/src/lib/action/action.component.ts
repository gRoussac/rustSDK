...
export class ActionComponent implements AfterViewInit, OnDestroy {
  ...
  async ngAfterViewInit(): Promise<void> {
    ...
    this.sdk_deploy_methods = this.sdk_methods.filter((name) =>
      [
        'deploy',
        'speculative_deploy',
    ...
    this.sdk_deploy_utils_methods = this.sdk_methods.filter((name) =>
      [
        'make_deploy',
        'make_transfer',
        'sign_deploy',
    ...
    this.sdk_transaction_methods = this.sdk_methods.filter((name) =>
      [
        'transaction',
    ...
    this.sdk_transaction_utils_methods = this.sdk_methods.filter((name) =>
      [
        'make_transaction',
    ...
    this.sdk_deprecated = this.sdk_methods.filter((name) =>
      [
        'get_deploy',
        'get_era_info',
        'put_deploy',
    ...
    this.sdk_binary_methods = this.sdk_methods.filter((name) =>
      name.startsWith('get_binary'),
    );
      ...
      ...new Set(
        this.sdk_binary_methods.filter((name) => {
          if (
            ...
            name.endsWith('_era') ||
            name.endsWith('_state_root_hash')
          ) {
            rejectedMethodsSet.add(name);
            const baseMethod = name.slice(0, name.lastIndexOf('_'));
            const final_name = baseMethod.replace(
              /(_by_block|_by_state_root|_by)$/,
              '',
            );
