...
export class HeaderComponent implements AfterViewInit {
  ...
  async ngAfterViewInit() {
    if (
      ...
      this.storageService.get('node_address')
    ) {
      this.chain_name =
      ...
      this.network =
        this.networks.find((x) => x.node_address == this.node_address) ||
        this.network;
    }
  ...
  onCustomNetworkChange($event: Event) {
    this.node_address =
      ($event.target as HTMLInputElement)?.value || this.network.node_address;
  ...
  private setNodeAddress() {
    if (this.is_electron) {
      ...
      const network = this.networks.find(
        (x) => x.node_address == this.node_address,
      );
      network &&
        this.sdk.setNodeAddress(
