...
| Actions / CEPS | `7`, `[`/`]`                   | Catalog of RPC + helpers; with `--features ceps`: full CEP-18/78/85/95 + CES + shared put/entrypoint via `ceps-client` (`ceps_client::wasm`); sticky binds; SDK `js` off | -                                                                                                     |
...
| Wait           | `9`, `/`, `w`, Space           | `wait_transaction`, `SSEClient::collect` (also used after CEP writes / CES)                                                                                              | `sdk_wait_transaction`, `sdk_SSE_collect`                                                             |
| Help           | `h`                            | -                                                                                                                                                                        | -                                                                                                     |
...
- CEP / CEPS write Actions (install, transfer, mint, …, `ceps_put_transaction`, `ceps_call_entrypoint`) use the same Put gate; see `allowed_ops` in `policy.sample.json`
...
# (or set wasm=cep18 / CEPS_CEP18_WASM)
```
